...
export default function PlayersManageScreen() {
  ...
  const addPlayer = async () => {
    ...
    await api.post("/players", { displayName: name.trim(), rating: value });
    setName("");
    setRating("5");
    await load();
  };
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.header}>
          <Text style={styles.kicker}>Manage roster</Text>
          <Text style={styles.title}>Players</Text>
          <Text style={styles.subtitle}>Add or remove players from your roster.</Text>
        </View>
